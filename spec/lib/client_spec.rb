...
describe Druid::Client do
  ...
  it 'calls zookeeper on intialize' do
    Druid::ZK.should_receive(:new)
    Druid::Client.new('test_uri', zk_keepalive: true)
  end

  it 'creates a query' do
    Druid::ZK.stub(:new).and_return(double(Druid::ZK, :data_sources => {'test/test' => 'http://www.example.com'}, :close! => true))
  ...
  it 'parses response on 200' do
    ...
    Druid::ZK.stub(:new).and_return(double(Druid::ZK, :data_sources => {'test/test' => 'http://www.example.com/druid/v2'}, :close! => true))
    client = Druid::Client.new('test_uri', zk_keepalive: true)
